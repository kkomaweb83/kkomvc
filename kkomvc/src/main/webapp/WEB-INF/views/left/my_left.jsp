...
<nav id="admin_menu_lnb">
	<ul>
		...
		<c:if test="${login.cmd == 101}"><a href="<%=cp %>/orders_list.do?dana=orders_list">주문 리스트</a></c:if>
		<li>
	</ul>
</nav>
...
<nav id="admin_menu_lnb">
	<ul>
		...
		<c:if test="${login.cmd == 101}"><a href="<%=cp %>/mem_preUpdate.do?dana=mem_preUpdate">회원정보 수정</a></c:if>
		<li>
	</ul>
</nav>
